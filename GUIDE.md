...
of PLambda/JLambda are:
   ...
   5. richer argument passing (apply vs kwapply).

     (kwapply f l d) is f(*l, **d)   f must be callable (i.e. a python function), our closures do not support kwapply.

   6. python things are already attributable (why do we need uids and fetch?)

   7. reflection in python has some holes in it (you cannot get the argspec of a
   built-in, so some guess work is required).
...
2. makes life very much easier: We have n-ary constructors mktuple,
mklist, and mkdict.
